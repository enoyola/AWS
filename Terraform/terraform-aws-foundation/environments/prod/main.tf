...
# EC2 Module - Production uses ASG by default
module "ec2" {
  source = "../../modules/ec2"
  ...
  subnet_ids            = module.vpc.private_subnet_ids  # Production in private subnets
  security_group_ids    = [
  ...
  create_asg           = true  # Always use ASG in production
  min_size             = var.asg_min_size
  max_size             = var.asg_max_size
  desired_capacity     = var.asg_desired_capacity
  target_group_arns    = var.create_load_balancer ? [module.load_balancer[0].target_group_arn] : []
  health_check_type    = var.create_load_balancer ? "ELB" : "EC2"
  user_data            = var.user_data
  enable_monitoring    = true  # Enable detailed monitoring in production
  common_tags          = local.common_tags
}

# RDS Module - Production with Multi-AZ and backups
module "rds" {
  ...
  deletion_protection     = true  # Always enable in production
...
# S3 Module - Production with lifecycle rules
module "s3" {
  count  = var.create_s3 ? 1 : 0
  source = "../../modules/s3"

  bucket_name        = "${var.project_name}-${local.environment}-${random_id.bucket_suffix.hex}"
  versioning_enabled = true  # Always enable versioning in production
  lifecycle_rules    = var.s3_lifecycle_rules
  common_tags        = local.common_tags
}

# Load Balancer Module - Production typically needs ALB
